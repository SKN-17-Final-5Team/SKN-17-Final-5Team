...
def main():
    DOCUMENT_PATH = "/Users/woojin/Desktop/무역_AI_Copliot/woojin/certification/certification.txt"
    QA_PATH = "/Users/woojin/Desktop/무역_AI_Copliot/woojin/certification/certif_retrieval_test.json"
    K = 5
    ...
    if not qa_list:
        print("[ERROR] span이 설정된 QA가 하나도 없습니다.")
        return
    ...
    for overlap in overlap_sizes:
        for tok_size in token_sizes:
            ...
            # 청크 생성
            chunks_tok = chunk_by_tokens(text, tok_size, overlap_ratio=overlap)

            # 컬렉션 이름에 overlap 포함
            col_name = f"tok_{tok_size}_ov_{int(overlap*100)}"
    ...
    print("\n============== FINAL SUMMARY ==============")
    print(f"{'strategy':25s} | {'overlap':>7} | {'Recall@'+str(K):>10} | {'MRR@'+str(K):>10} | {'no_rel':>7}")
    print("-----------------------------------------------------------------------")
    for r in results:
        print(
            f"{r['strategy']:25s} | "
