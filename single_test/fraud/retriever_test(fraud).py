...
def upload_chunks_to_qdrant(client: QdrantClient, collection_name: str, chunks):
    ...
    # [수정 1] 텍스트가 비어있지 않은 유효한 청크만 필터링 (Index Alignment 유지)
    valid_chunks = [c for c in chunks if c["text"] and c["text"].strip()]
    
    texts = [c["text"] for c in valid_chunks]
    print(f"[QDRANT] 임베딩 계산 대상 chunk 수: {len(texts)} (원본: {len(chunks)})")
    
    if not texts:
        print("[QDRANT] 유효한 텍스트가 없어 업로드를 중단합니다.")
        return

    # 이제 texts와 valid_chunks의 개수가 정확히 일치함
    ...
    total_points = 0
    # chunks 대신 valid_chunks를 사용
    for start in range(0, len(valid_chunks), UPSERT_BATCH_SIZE):
        end = min(start + UPSERT_BATCH_SIZE, len(valid_chunks))
        ...
        # [수정 2] points가 비어있으면 업로드 스킵 (에러 방지)
        if not points:
            print(f"[QDRANT] 배치 {start} ~ {end-1}: 업로드할 포인트가 없어 건너뜁니다.")
            continue
...
def main():
    DOCUMENT_PATH = "/Users/woojin/Desktop/무역_AI_Copliot/woojin/fraud/2025무역사기예방및대응매뉴얼.md"
    QA_PATH = "/Users/woojin/Desktop/무역_AI_Copliot/woojin/fraud/fraud_test.json"
    K = 5
